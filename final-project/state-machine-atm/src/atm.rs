...
impl StateMachine for Atm {
    ...
    fn next_state(starting_state: &Self::State, t: &Self::Transition) -> Self::State {
        match (starting_state.expected_pin_hash, t) {
            (Auth::Waiting, Action::SwipeCard(pin_hash)) => Atm {
                expected_pin_hash: Auth::Authenticating(*pin_hash),
                ..starting_state.clone()
            },

            (Auth::Authenticating(_), Action::SwipeCard(pin_hash)) => Atm {
                expected_pin_hash: Auth::Authenticating(*pin_hash),
                ..starting_state.clone()
            },

            (Auth::Authenticating(_), Action::PressKey(key)) => match key {
                Key::Enter => {
                    let pin_hash = crate::traits::hash(&starting_state.keystroke_register);
                    if Auth::Authenticating(pin_hash) == starting_state.expected_pin_hash {
                        return Atm {
                ...
                _ => {
                    let mut keystroke_register_clone = starting_state.keystroke_register.clone();
                    keystroke_register_clone.push(*key);

                    Atm {
                        cash_inside: starting_state.cash_inside,
                        expected_pin_hash: starting_state.expected_pin_hash,
                        keystroke_register: keystroke_register_clone,
                    }
                }
            },

            (Auth::Authenticated, Action::PressKey(key)) => match key {
                Key::Enter => {
                    let mut amount = 0;
                    for key in &starting_state.keystroke_register {
                        let key_str: &str = From::from(*key);
                        let number = key_str.parse::<u64>().unwrap();
            ...
            _ => starting_state.to_owned(), // Default behavior is to remain in the same state
        }
    }
}
